...
import { Check, Edit2, ArrowRight, Wand2, Loader2 } from 'lucide-react';
...
export function ReviewView({ images, folderName, onProceed }: ReviewViewProps) {
  ...
  const handleSectionChange = (imageId: string, newSection: string) => {
    setLocalImages(prev => prev.map(img => 
      img.id === imageId ? { ...img, section: newSection } : img
    ));
  ...
  const handleAiEdit = async (img: ImageItem) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.imageBase64) {
          const newPreviewUrl = `data:\${data.mimeType};base64,\${data.imageBase64}`;
          setLocalImages(prev => prev.map(item => 
            item.id === img.id ? { ...item, previewUrl: newPreviewUrl } : item
          ));
        }
      } else {
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 border-b border-white/10 pb-8">
        <div>
          ...
          <p className="text-white/60 font-light">
            Album: <span className="font-medium text-[#D4AF37]">{folderName}</span>. AI has sorted your photos. You can move them or edit them with AI.
          </p>
        </div>
        <button
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
              {section.images.map((img) => (
                <div key={img.id} className="group relative aspect-square rounded-3xl overflow-hidden bg-zinc-800 border border-white/5">
                  ...
                  {/* Overlay for editing */}
                  <div className={cn("absolute inset-0 bg-black/60 opacity-0 transition-opacity duration-200 p-2 flex flex-col justify-end", 
                    ...
                    {aiEditingFor === img.id ? (
                      ...
                      <div className="bg-zinc-900 border border-white/10 rounded-2xl p-2 shadow-xl" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="max-h-32 overflow-y-auto space-y-1">
                          {allSectionNames.filter(n => n !== section.name).map(n => (
                            <button
                              key={n}
